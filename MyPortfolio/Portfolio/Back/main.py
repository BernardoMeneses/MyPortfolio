...
GITHUB_REDIRECT_URI = os.getenv("GITHUB_REDIRECT_URI", "http://localhost:5173/github/callback.html")
...
def get_db_connection():
    ...
    if not os.path.exists(DATABASE_PATH):
        raise HTTPException(
            status_code=500, 
            detail="Base de dados não encontrada. Execute 'python init_db.py' primeiro."
        )
...
def add_recommendation_to_db(name: str, text: str, avatar: str, username: str, provider: str = 'github'):
    """Adicionar recomendação à base de dados"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    cursor.execute('''
        INSERT INTO recommendations (name, text, avatar, username, provider)
        VALUES (?, ?, ?, ?, ?)
    ''', (name, text, avatar, username, provider))
...
def get_recommendations_from_db():
    ...
    cursor.execute('''
        SELECT name, text, avatar, username, provider, created_at
...
def add_project_to_db(title: str, description: str, tech: List[str], repo: str, image: str):
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?)
    ''', (title, description, json.dumps(tech), repo, image))
...
def get_projects_from_db():
    ...
    cursor.execute('''
        SELECT title, description, tech, repo, image, created_at
        FROM projects
        ORDER BY created_at DESC
    ''')
...
def save_contact_message(name: str, email: str, message: str, sent_by_email: bool = False):
    """Salvar mensagem de contato na base de dados"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    cursor.execute('''
        INSERT INTO contact_messages (name, email, message, sent_by_email)
        VALUES (?, ?, ?, ?)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://localhost:4698"],
...
async def github_callback(code: str, state: str):
    ...
    try:
        ...
        if not GITHUB_CLIENT_ID or not GITHUB_CLIENT_SECRET:
            error_url = f"http://localhost:5173/callback.html?error=config_error"
            return RedirectResponse(url=error_url)
        
        # Verificar se o state é válido
        if state not in oauth_states:
            error_url = f"http://localhost:5173/callback.html?error=invalid_state"
            return RedirectResponse(url=error_url)
        ...
        if response.status_code != 200:
            error_url = f"http://localhost:5173/github/callback.html?error=token_exchange_failed"
            return RedirectResponse(url=error_url)
        ...
        if not access_token:
            error_url = f"http://localhost:5173/github/callback.html?error=no_access_token"
            return RedirectResponse(url=error_url)
        ...
        if user_response.status_code != 200:
            error_url = f"http://localhost:5173/github/callback.html?error=user_data_failed"
            return RedirectResponse(url=error_url)
        ...
        success_url = f"http://localhost:5173/github/callback.html?{urllib.parse.urlencode(user_params)}"
        return RedirectResponse(url=success_url)
        
    except Exception as e:
        error_url = f"http://localhost:5173/github/callback.html?error=server_error&message={str(e)}"
        return RedirectResponse(url=error_url)
...
async def google_callback(code: str, state: str):
    ...
    try:
        ...
        if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
            error_url = f"http://localhost:5173/google/callback.html?error=config_error"
            return RedirectResponse(url=error_url)
        
        # Verificar se o state é válido
        if state not in oauth_states:
            error_url = f"http://localhost:5173/google/callback.html?error=invalid_state"
            return RedirectResponse(url=error_url)
        ...
        if response.status_code != 200:
            error_url = f"http://localhost:5173/google/callback.html?error=token_exchange_failed"
            return RedirectResponse(url=error_url)
        ...
        if not access_token:
            error_url = f"http://localhost:5173/google/callback.html?error=no_access_token"
            return RedirectResponse(url=error_url)
        ...
        if user_response.status_code != 200:
            error_url = f"http://localhost:5173/google/callback.html?error=user_data_failed"
            return RedirectResponse(url=error_url)
        ...
        success_url = f"http://localhost:5173/google/callback.html?{urllib.parse.urlencode(user_params)}"
        return RedirectResponse(url=success_url)
...
async def add_project(project: Project):
    ...
    try:
        ...
        return {"message": "Projeto adicionado com sucesso!", "project_id": project_id}
...
def get_stats():
    ...
    # Contar mensagens enviadas com sucesso
    cursor.execute("SELECT COUNT(*) as count FROM contact_messages WHERE sent_by_email = 1")
    sent_messages_count = cursor.fetchone()["count"]
    ...
    return {
        ...
        "contact_messages_count": messages_count,
